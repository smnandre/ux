...
namespace Symfony\UX\TwigComponent\Tests\Unit\Twig\Processor;

use PHPUnit\Framework\TestCase;
use Symfony\UX\TwigComponent\Twig\Processor\HtmlSyntaxProcessor;
use Twig\Error\SyntaxError;

final class HtmlSyntaxProcessorTest extends TestCase
{
    /**
     * @dataProvider getValidSyntaxTests
     */
    public function testProcessValidSyntax(string $input, string $expectedOutput): void
    {
        $processor = new HtmlSyntaxProcessor();
        $this->assertSame($expectedOutput, $processor->process($input));
    }

    /**
     * @dataProvider getInvalidSyntaxTests
     */
    public function testThrowsExceptionOnInvalidSyntax(string $input, string $expectedMessage): void
    {
    ...
    public static function getValidSyntaxTests(): iterable
    {
        ...
        yield 'not_simple_component' => [
            "<twig:foo /> a <twig:bo>a</twig:bo>",
            '{{ component("foo") }} a {% component "bo" %}a{% endcomponent %}',
        ];
