...
The limited data query stays lazy until Twig iterates the result. This example
opts into ``throwOnOutOfRange()``, so ``paginate()`` runs the exact count
query immediately and rejects a page past the total with a 404. Without that
opt-in, the count stays lazy until totals or numbered navigation need it.
...
.. code-block:: text
    ...
    GET /products?category=books&page=2
      ...
      7. the Doctrine adapter receives offset 12 and limit 12
      8. throwOnOutOfRange() runs the exact count query
      9. Twig iteration runs the limited data query
     10. PaginationUrlGenerator builds links from the current Request
     11. category=books is preserved and page changes for each link
     12. Twig renders a complete <nav> with ordinary links
