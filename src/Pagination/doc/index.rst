...
Every strategy follows the same request/response sequence:

.. code-block:: text

    Request (?page= or ?cursor=)
        -> immutable paginator builder
        -> source adapter (Doctrine, array or custom)
        -> lazy pagination result
        -> Twig renders ordinary links
        -> browser performs a normal navigation
...
Use this decision path when the choice is not obvious:
...
The package supports PHP 8.4+, Symfony 7.4+ and 8.0+, and Twig 3.21 or higher.
