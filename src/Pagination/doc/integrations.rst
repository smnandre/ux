...
.. code-block:: json

    {
        ...
        "links": {
            "prev": null,
            "next": "/events?cursor=opaque-signed-token"
        }
    }
...
Keep the GitHub client and its authentication in the application. Implement
``CursorAdapterInterface`` for an application source such as
``GitHubPullRequestConnection`` and map the cursor contract directly:
