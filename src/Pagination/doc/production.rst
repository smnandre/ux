...
Cursor field names are validated against metadata, but a fixed
allow-list at the controller boundary is clearer for all strategies.

Cursor tokens are signed, not encrypted. They are limited to 4096 bytes
and 16 ordered scalar values. Treat them as opaque, keep confidential
data out of the context, and expect secret rotation or query changes to
invalidate them.
...
querying the adapter. Cursor queries fetch at most ``perPage + 1`` rows.
...
* Keep ``kernel.secret`` stable across every application instance.
