...
Add first and last controls         Override ``navigation`` and reuse its blocks
...
Three attribute groups cover the common extension points:

========================== ================================================
Option                     Target
========================== ================================================
``attributes``             Root ``<nav>`` element
``navigationAttributes``   Inner list containing the controls
``linkAttributes``         Previous, numbered and next links
========================== ================================================

The renderer appends ``class`` values to the classes supplied by the selected
theme. Any other attribute replaces the theme default with the same name:
...
The explicit ``attributes`` map is also accepted:
...
``linkAttributes`` also accepts a PHP closure. It runs once for each link and
...
The renderer validates attribute names and value types before rendering. Twig
escapes every value, boolean attributes are supported, and no value is
rendered through ``raw``. ``linkAttributes`` cannot replace ``href``: the
pagination result keeps ownership of navigation URLs.
...
The renderer looks for a ``pagination`` block. When the template defines one,
only that block renders: the label and attribute blocks are helpers called
through the ``block()`` function and never render on their own. A template
without a ``pagination`` block renders as a whole, so a standalone
application theme stays a plain Twig template.
...
``navigationAttributes``       Validated controls attributes
``linkAttributes.previous``    Resolved previous-link attributes
``linkAttributes.pages``       Resolved attributes indexed by page number
``linkAttributes.next``        Resolved next-link attributes
``showInfo``                   Whether the summary was requested
============================== ==============================================

``linkAttributes.pages`` contains only numbered pages rendered as links. The
...
``No items``
    Empty result summary, shared by both strategies.
...
.. code-block:: html+twig
    ...
    {{ ux_pagination(products, showInfo: false) }}
