...
instead. See :doc:`custom-adapters` and the complete GitHub GraphQL
example in :doc:`integrations`.
...
* the ordered fields and direction;
* the Doctrine root entity, DQL and normalized query parameters;
* an optional application context.
...
Tokens are authenticated, not encrypted. Treat them as opaque transport
values and never put confidential information in the context.

Non-Doctrine sources require an explicit, stable context::
...
cursors. Custom cursor adapters use ``cursor()->context()`` for the same
purpose.
...
Changing the effective cursor secret, the query, its parameters, the
context or the effective order invalidates existing tokens by design.
