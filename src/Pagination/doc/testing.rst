...
proves that the repository order and its unique tie-breaker prevent
duplicates and gaps::
...
Keep repository fixtures small and deterministic. This example filters eight
products down to a five-item first page and a three-item second page, so the
state and DOM assertions prove that ``nextPage`` re-ran the server query.
