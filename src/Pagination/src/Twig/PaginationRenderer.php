...
final class PaginationRenderer
{
    ...
    public function renderPagination(
        ...
        $context = [
            ...
            'navigationAttributes' => $this->normalizeAttributes($navigationAttributes, 'navigationAttributes'),
            'showInfo' => $showInfo,
            'numbered' => $numbered,
            'linkAttributes' => $this->resolveLinkAttributes($pagination, $linkAttributes),
        ];

        // Themes structured in blocks (like the built-in ones) render through
        // their "pagination" block, so their label and attribute blocks never
        // leak into the output. Standalone templates render as a whole.
        $template = $this->twig->load($theme);
        if ($template->hasBlock('pagination', $context)) {
            return $template->renderBlock('pagination', $context);
        }
